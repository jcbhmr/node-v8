...
function bytesNeededForVariantUint32(value: number) {
    let result = 0;
    do {
        result++
        value >>= 7
...
class ValueSerializer {
    ...
    writeHeader() {
        ...
        this.writeVariant(LATEST_VERSION)
    }
    ...
    writeTwoByteString(chars: Uint16Array) {
        ...
        this.writeRawBytes(new Uint8Array(chars.buffer, chars.byteOffset, chars.byteLength))
    }
    ...
    expandBuffer(requiredCapacity: number) {
        ...
        let newBuffer = null
        if (this.delegate) {
            [newBuffer, providedCapacity] = this.delegate.reallocateBufferMemory(this.buffer, requestedCapacity, providedCapacity)
        } else {
    ...
    release() {
        const result = [this.buffer, this.bufferSize]
        this.buffer = null
        this.bufferSize = 0
    ...
    writeObject(object: any) {
        ...
        if (this.isSmi(object)) {
            this.writeSmi(object)
            this.throwIfOutOfMemory()
            return
        }

        const instanceType = getInstanceType(object)
        switch (instanceType) {
            case "ODDBALL_TYPE":
            ...
            case "JS_RAB_GSAB_DATA_VIEW_TYPE": {
                ...
                if (!this.idMap.get(view) && !this.#treatArrayBufferViewsAsHostObjects) {
            ...
            default:
                if (instanceTypeCheckerIsString(instanceType)) {
                    this.writeString(object)
                    this.throwIfOutOfMemory()
                    return
                } else if (instanceTypeCheckerIsString(instanceType)) {
                    return writeJSReceiver(object)
                } else {
    ...
    writeString(string: string) {
        ...
        if (flat.isOneByte()) {
            ...
            if ((bufferSize + 1 + bytesNeededForVariantUint32(byteLength)) & 1) {
    ...
    writeJSReceiver(receiver: any) {
        ...
        const instanceType = getInstanceType(receiver)
        if (isCallable(receiver) || (isSpecialReceiverInstanceType(instanceType) && instanceType !== "JS_SPECIAL_API_OBJECT_TYPE")) {
        ...
        switch (instanceType) {
            case "JS_ARRAY_TYPE":
                ...
                case "JS_API_OBJECT_TYPE": {
                    ...
                    const isHostObject = isHostObject(jsObject)
